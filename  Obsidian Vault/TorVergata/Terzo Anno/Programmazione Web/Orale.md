...
In una funzione, il valore di this dipende da come la funzione è chiamata. Quando una funzione è chiamata come metodo di un oggetto, this si riferisce all'oggetto stesso. Quando una funzione è chiamata come funzione standalone, this si riferisce all'oggetto globale (window in broser, global in Node.js) o undefined in strict mode.
...
## Asincronia, Event Loop, Promise e code

#### Sincrono vs Asincrono
Sincrono *esegue istruzioni in sequenza bloccante*, asincrono *permette di non bloccare usando* callback/Promise/async-await
...
#### Event Loop
Meccanismo che gestisce l'esecuzione asincrona controllando call stack e code, eseguendo callback quando lo stack è libero
...
Coda dove finiscono molte callback/eventi in attesa che lo stack sia vuoto

#### Microtask queue
Coda ad alta priorità eseguito subito dopo il task corrente

#### Microtask vs macro-task
Le microtask "passano avanti" e vengono svuotate prima di riprendere con i task normali
...
#### Node.js
Runtime JavaScript basato su V8 che permette di eseguire Js lato server, orientato agli eventi e con I/O non bloccante
...
#### Middleware
Funzione che riceve *req, res, next* e deve chiamare *next( )* per passare al middleware successivo

#### Routing
Associa URl a funzioni/handler che rispondono alle richieste; può essere statico, dinamico o annidato
